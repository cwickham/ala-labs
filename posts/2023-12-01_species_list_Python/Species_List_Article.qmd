...
In this post, we will use the `galah-python`, `geopandas`, `matplotlib` and `pywaffle` packages to show you how to download a list of species within the Yass Valley in 2023, and visualise the number of threatened and sensitive species in the region. 
...
Now that `yass_geometry` contains our LGA shape, we can build our query. We can use `atlas_species()` to return a species list, passing our shape to the `polygon` argument of `atlas_species()` and specifying the year to `filters`. To add the observation count for each species, add `counts=True`.
...
Each method both use the same method of matching species names on our Yass Valley list to names on official conservation status lists. However, there is a slightly different workflow between using `galah-python` and using an externally downloaded list. Choose from the options below to use either method.
...
In this case, we can use list ID `dr650` to narrow our species list to only those also on the New South Wales Conservation List by adding `species_list_uid=dr650` to `filters`. Our query shows that there were 21 threatened species in Yass Valley in 2023.
...
match names in `threatened_list`. We return 21 species.
